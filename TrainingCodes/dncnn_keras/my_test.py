...
def parse_args():
    ...
    parser.add_argument('--set_dir', default='data/Volume2', type=str, help='directory of test dataset')
    parser.add_argument('--set_names', default=['val'], type=list, help='name of test dataset')
    parser.add_argument('--sigma', default=25, type=int, help='noise level')
    parser.add_argument('--model_dir', default=os.path.join('models', 'MyDenoiserMedium'), type=str,
...
def main():
    ...
    for set_cur in args.set_names:
        ...
        for image in os.listdir(os.path.join(args.set_dir, set_cur, 'CoregisteredBlurryImages')):
            if image.endswith(".jpg") or image.endswith(".bmp") or image.endswith(".png"):

                # Load the clear (Coregistered) image x, and normalize the pixel values to be between 0 and 1
                x = np.array(imread(os.path.join(args.set_dir, set_cur, 'ClearImages', image)),
                             dtype=np.float32) / 255.0

                # Load the blurry image y, and normalize the pixel values to be between 0 and 1
                y = np.array(imread(os.path.join(args.set_dir, set_cur, 'CoregisteredBlurryImages', image)),
                             dtype=np.float32) / 255.0
                ...
                print('%10s : %10s : %2.4f second' % (set_cur, image, elapsed_time))
                x_ = from_tensor(x_)
