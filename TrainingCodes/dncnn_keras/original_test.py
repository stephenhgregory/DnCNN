...
def parse_args():
    ...
    parser.add_argument('--model_dir', default=os.path.join('models', 'DnCNN_sigma25'), type=str, help='directory of '
                                                                                                       'the model')
...
if __name__ == '__main__':
    ...
    for set_cur in args.set_names:  
        ...
        for im in os.listdir(os.path.join(args.set_dir,set_cur, 'ClearImages')):
            if im.endswith(".jpg") or im.endswith(".bmp") or im.endswith(".png"):
                x = np.array(imread(os.path.join(args.set_dir, set_cur, 'ClearImages', im)), dtype=np.float32) / 255.0
                y = np.array(imread(os.path.join(args.set_dir, set_cur, 'CoregisteredBlurryImages', im)), dtype=np.float32) / 255.0
                ...
                if args.save_result:
                    ...
                    show(np.hstack((y,x_, x))) # show the image
