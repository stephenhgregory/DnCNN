...
def DnCNN(depth,filters=64,image_channels=1, use_bnorm=True):
    ...
    # 1st layer, Conv+relu
    layer_count += 1
    x = Conv2D(filters=filters, kernel_size=(3,3), strides=(1,1),kernel_initializer='Orthogonal', padding='same',name = 'conv'+str(layer_count))(input)
    layer_count += 1
    ...
    layer_count += 1
    x = Subtract(name = 'subtract' + str(layer_count))([input, x])   # input - noise
